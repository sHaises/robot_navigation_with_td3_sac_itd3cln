...
class SAC:
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005, alpha=0.2):
        for it in range(iterations):
            ...
            # Calculează av_Q și max_Q
            av_Q += (current_q1.mean() + current_q2.mean()).item()
            max_Q = torch.max(target_q1, target_q2).mean().item()

            # Scrie pierderea criticului, a value-ului și a actorului în TensorBoard
        self.writer.add_scalar("loss", av_loss / (it + 1), self.iter_count)
        self.writer.add_scalar("Av. Q", av_Q / (it + 1), self.iter_count)
        self.writer.add_scalar("Max. Q", max_Q, self.iter_count)

        self.iter_count += 1

            # Actualizează mesajul de jurnalizare
        env.get_logger().info(f"writing new results for a tensorboard")
        env.get_logger().info(f"loss, Av.Q, Max.Q, iterations : {av_loss / (it + 1)}, {av_Q / (it + 1)}, {max_Q}, {self.iter_count}")
